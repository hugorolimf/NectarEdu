...
```typescript
import { apiClient, safeRequest, NectarEDU } from '$lib/utils/services/api';
...
// Use with Hono RPC
const data = await NectarEDU.users.$get();
const user = await NectarEDU.users.$post({ json: { name: 'John' } });
```
