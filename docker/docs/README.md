# Publishing NectarEDU Docker Images to Docker Hub

This guide explains how to publish the NectarEDU API and Dashboard Docker images to Docker Hub.
...
```bash
# Default (uses 'NectarEDU' as username and 'latest' as version)
...
DOCKERHUB_USERNAME=NectarEDU VERSION=v0.1.0 ./docker/docker-push.sh
```

### Step 3: Verify Images

Check your Docker Hub repository at:
- https://hub.docker.com/r/NectarEDU/api
- https://hub.docker.com/r/NectarEDU/dashboard
...
services:
  ...
  dashboard:
    image: NectarEDU/dashboard:latest
    # Remove the 'build' section
    ports:
      - "3082:3082"
    environment:
...
docker build -f docker/Dockerfile.api \
  -t NectarEDU/api:latest \
  -t NectarEDU/api:v1.2.0 \
  -t NectarEDU/api:v1.2 \
  -t NectarEDU/api:v1 \
  .

# Push all tags
docker push NectarEDU/api:latest
docker push NectarEDU/api:v1.2.0
docker push NectarEDU/api:v1.2
docker push NectarEDU/api:v1
```
...
4. **Scan images** for vulnerabilities:
   ```bash
   docker scan NectarEDU/api:latest
   ```
